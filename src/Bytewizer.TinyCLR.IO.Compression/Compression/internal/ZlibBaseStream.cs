	...
	internal class ZlibBaseStream : Stream
	{
		...
		public override void Write(byte[] buffer, int offset, int count)
		{
			...
			do
			{
				...
				int num = (WantCompress ? _z.Deflate(_flushMode) : _z.Inflate(_flushMode));
				if (num != 0 && num != 1)
				{
					throw new Exception((WantCompress ? "de" : "in") + "flating: " + _z.Message);
				}
		...
		private void Finish()
		{
			...
			if (_streamMode == StreamMode.Writer)
			{
				...
				do
				{
					...
					_z.NextOut = 0;
					_z.AvailableBytesOut = _workingBuffer.Length;
					int num = (WantCompress ? _z.Deflate(FlushType.Finish) : _z.Inflate(FlushType.Finish));
					if (num != 1 && num != 0)
					{
						string text = (WantCompress ? "de" : "in") + "flating";
						if (_z.Message == null)
						{
		...
		public override int Read(byte[] buffer, int offset, int count)
		{
			...
			do
			{
				...
				num = (WantCompress ? _z.Deflate(_flushMode) : _z.Inflate(_flushMode));
				...
				if (num != 0 && num != 1)
				{
					throw new Exception(string.Format("{0}flating:  rc={1}  msg={2}", WantCompress ? "de" : "in", num, _z.Message));
				}
			}
