    ...
    public class ZipInputStream : InflaterInputStream
    {
        ...
        public ZipEntry GetNextEntry()
        {
            ...
            if (isCrypted)
            {
                ...
                if ((flags & 8) == 0)
                {// -jr- 10-Feb-2004 Dont yet know correct size here....
                    csize -= 12;
                }
            }
            else
        ...
        public override int Read(byte[] b, int off, int len)
        {
            ...
            switch (method)
            {
                case ZipOutputStream.DEFLATED:
                    ...
                    if (len <= 0)
                    { // TODO BUG1 -jr- Check this was < 0 but avail was not adjusted causing failure in later calls
                        if (!inf.IsFinished)
                        {
